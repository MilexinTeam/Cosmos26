...
rem Ensure ProgramFiles(x86) exists (for 32-bit-less systems)
...
rem --- FIX: no version filter, works with VS 2017 → 2026 ---
...
if not exist "%MSBuild%" (
  echo MSBuild not found. Please make sure Visual Studio with MSBuild component is installed.
  pause
  goto:eof
)
...
if "%update%"=="0" (
    echo The update KB3140245 is not installed! It is required on Windows 7 to build Cosmos.
    pause
)
